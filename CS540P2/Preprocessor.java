...
public class Preprocessor {
    ...
    /* Convert img files in a directory_path to a csv file with y value at the beginning of line */
    ...
    private static List<String> resize_then_parse(File input) throws IOException {
        ...
        if (name.contains("a.jpg")) record.add("0");
        else if (name.contains("b.jpg")) record.add("1");
        else return null;
    ...
    private static BufferedImage resize(BufferedImage img, int height, int width) {
        Image tmp = null;
        if (img != null) tmp = img.getScaledInstance(width, height, Image.SCALE_SMOOTH);
